...
def available(joystickNumber = 0):
    """Проверьте, подключен ли джойстик и готов ли он к использованию."""
...
class Gamepad:
    ...
    class UpdateThread(threading.Thread):
        """Поток, используемый для постоянного запуска функции updateState на геймпаде в фоновом режиме
         Один из них создается функцией startBackgroundUpdates на геймпаде и закрывается с помощью stopBackgroundUpdates"""
...
###############################
# Импорт отображений геймпада #
###############################
scriptDir = os.path.dirname(os.path.realpath(__file__))
""" Функция os.path.dirname возвращает только часть каталога пути. Мы просто возвращаем путь к каталогу.
    os.path.realpath(__file__) - возвращает канонический путь, убирая все символические ссылки (если они поддерживаются). """
controllerScript = os.path.join(scriptDir, "Controllers.py")
""" Метод os.path.join позволяет вам совместить несколько путей при помощи присвоенного разделителя. """
exec(open(controllerScript).read())
""" Функция exec () принимает код Python и запускает функцию open () для загрузки скрипта. """

# Создайте список доступных типов геймпада
moduleDict = globals()
"""globals - возвращает словарь с глобальной таблицей символов, определённых в модуле.
Глобальная таблица символов хранит всю информацию, относящуюся к глобальной области действия программы """
...
""" 
    ...
    inspect.isclass(moduleDict[a])
    Возвращает True, если объект moduleDict[a] является классом.
    inspect позволяет проверять живые объекты, а итерация по всем локальным переменным позволяет проверять все в текущем пространстве имен.
"""

controllerDict = {} #Словарь
deviceNames = [] #Список
...
#########################################################################
# Когда этот скрипт запущен, он предоставляет тестовый код для геймпада #
#########################################################################
...
if __name__ == "__main__":
    ...
    # Последовательности цветовых кодов ANSI
    GREEN = '\033[0;32m'
    CYAN = '\033[0;36m'
    BLUE = '\033[1;34m'
    RESET = '\033[0m'

    # Спросите геймпад для использования
...
#radio.openReadingPipe(1, pipies[1])
radio.openWritingPipe(pipes[1])# Открываем трубу с адресом [0xc2, 0xc2, 0xc2, 0xc2, 0xc2] для передачи данных (передатчик может одновременно вещать только по одной трубе).
...
while True:
    """
        # Отображать сообщения о событиях по мере их поступления
        while True:
    ...
    """
    eventType, index, value = gamepad.getNextEvent()
    message = list(str(index)+' '+ str(value))
    radio.write(message)
    print("Мы отправили сообщение {}".format(message))

    # Check if it returned a ackPL
    if radio.isAckPayloadAvailable():
        returnedPL = []
        radio.read(returnedPL, radio.getDynamicPayloadSize())
        print("Наш возвращенный груз был {}".format(returnedPL))
    else:
        print("Полезная нагрузка не получена")
